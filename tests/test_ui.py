...
def test_stats_shape_today_and_days(client):
    ...
    data = resp.json()
    assert "today" in data and "days" in data
...
def test_origin_thread_resolved(client, monkeypatch):
    ...
    from app import main
    monkeypatch.setattr(main, "session_thread_map", lambda sids: {s: "4721" for s in sids})
    payload = _hermes_payload()
    body = json.dumps(payload).encode()
    client.post(
        "/v1/adapters/hermes",
    ...
    row = client.get("/api/events?per_page=10").json()["events"][0]
    assert row["origin_thread"] == "4721"


def test_meta_endpoint_thread_names(client, monkeypatch):
    monkeypatch.setattr(settings, "thread_names", {"4721": "NotifyGate"})
    resp = client.get("/api/meta")
    assert resp.status_code == 200
    assert resp.json()["thread_names"]["4721"] == "NotifyGate"
